...
| **Design notes scan** | `.claude/skills/.../scripts/designNotesScan.js` | Counts what the JSON can't: hero composition, photography, headline size, stat blocks, press logos… by reading the per-site `design.md`. Reports `unknown` rather than guessing |
...
| **Commands** | `.claude/commands/` | `/design` (+ `/crea-design`, Italian alias) |
...
data/datasets/<category>/          measured datasets — 3 samples included, add yours with dataset-builder
