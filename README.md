...
| **Commands** | `.claude/commands/` | `/design` (+ `/crea-design`, Italian alias) · `/publish-data` and `/publish-screenshots` for shipping a finished file to the Figma Community |
...
| **Test fixture** | `test/fixtures/datasets/` | The text half of one real dataset (no screenshots) so CI asserts the counting scripts against measured data — `data/datasets/` is gitignored |
...
data/datasets/<category>/          measured datasets — generated locally by dataset-builder (gitignored)
test/fixtures/datasets/            frozen text-only fixture so CI can assert real counts
