...
| **Build gate** | `.claude/hooks/buildGate.js` | `PreToolUse` hook that **refuses the Figma build** until the run manifest shows the counting and originality gates ran. The harness executes it, not the model — which is the only enforcement here that does not depend on the model choosing to comply. An escape hatch lifts it for non-design work, and leaves a file saying so |
