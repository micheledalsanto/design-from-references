...
## 2026-09-04 — jobBoards / Open Call: the corpus overturned a rule in this file

First run on a category built from scratch inside the run itself
(`dataset-builder`, 12 live sites, 4 clusters). Cluster: Playful Saturated /
Illustrated. Deliverable: Figma, 7 screens, 10 components, 40 variants.
**Not a rejection — the design passed the Early Look Gate on the first
showing.** Recorded because the run changed a rule.

- **`antiSlop.md` was recommending the minority practice while citing the
  count that contradicted it.** Its type paragraph said "pair two voices as the
  default", resting on a corpus that then split roughly even. Adding jobBoards
  — 11 of its 12 sites run a single family — tipped the corpus to **64
  single-family against 54 two-family**. The default had become wrong, and the
  only reason anyone noticed is that `houseStyleTally.js --check` exits 1 when
  the prose and the corpus disagree, and `npm run dataset:backup` made the new
  category part of the corpus in the same sitting. Fixed: there is now no
  corpus-wide default to inherit, only the cluster's own TYPE PAIRING row.
- **What the counts caught that the house style would have got wrong.** Three
  binding verdicts in this cluster run *against* the agent's defaults, and all
  three came from the scripts rather than from taste: headlines belong **at or
  below 56px** (48 / 40 / 49 against one outlier at 72), there are **no stat
  blocks** (3 of 4 have none), and **radius must vary by role** (3 of 4 vary
  it). Big type, a stat row and one cloned radius would each have passed every
  gate this skill had before `designNotesScan.js` existed.
- **The deviation gate did its job by having exactly one answer.** Only
  OPENING SECTION came back OPEN, with four different answers across four
  sites. Everything else was LAW or NORM and was conformed to. Spending the
  single deviation there — the board opens on the craft index and the search
  instead of a marketing hero — is the whole of what makes the design not the
  category mean.
- **Two build faults, both the same documented gotcha.** `resize()` after
  setting `primaryAxisSizingMode = 'AUTO'` pins the axis back to FIXED: it
  collapsed the Nav and Footer components to 10px, and then the Field, FAQ row,
  Empty state and skeleton the same way, which only showed up as a wrecked
  Apply form. `figma-use` already documents the ordering. **Set the sizing mode
  AFTER the resize, every time.**
- **A third fault the gotcha list does not cover:** a `LINE` inside a
  zero-height auto-layout frame is **clipped out of existence** — the
  perforation, the signature element of the whole design, rendered as nothing
  while reporting correct strokes and a correct dash pattern. `clipsContent =
  false` on the row fixed it. Worth adding to the frame-fill sweep: the default
  white fill and the default clipping are two faces of the same trap.

**Lesson: a run that ships without a rejection can still be the most useful
one, because the corpus it adds is what audits the prose. The type-pairing
default survived eleven categories not because it was right but because nothing
had counted it lately — and it fell within minutes of the twelfth being backed
up and the check being run. Regenerate and re-check after every dataset build,
