...
- **Five of the dataset steps were vacuous or crashing.** `data/datasets/` is
  ...
  never execute. Fixed by committing `test/fixtures/datasets/`, the text half
  of one real dataset at ~100 KB, and pointing the assertions at it via
  `--dataset-root`. Verified by cloning to a clean tree and running all 11
  steps, then mutation testing: deleting the fixture and corrupting a
  `design.md` both correctly fail.
- **CONTRIBUTING asked for a PR git silently discards.** It says to open a PR
  with `dataset.json` + the `design.md` files, while `.gitignore` excludes all
  of `/data/datasets/`. A contributor following the instructions produces an
  empty PR with no warning. Documented with the `git add -f` workaround rather
  than narrowing the ignore rule, since committing third-party site data is the
  maintainer's call, not a documentation fix.
