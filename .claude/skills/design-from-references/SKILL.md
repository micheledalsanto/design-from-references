...
Measured on 2026-09-04 by `houseStyleTally.js` across **12 categories,
118 sites**. Regenerate with `node <skill root>/scripts/houseStyleTally.js --write`
...
| --- | --- | --- |
| dark / near-black background | light wins in **12/12** categories (7-0, 10-1, 7-3, 7-3, 8-2, 9-1, 8-4, 7-1, 9-3, 9-1, 9-0, 6-3) | never the majority anywhere; check the per-category count before treating it as absolute |
| fully desaturated or B&W photography | **62 colour vs 3 desaturated** where the notes state it | sites with no photography at all are excluded from both columns |
| italicised keyword in a headline | **6/118** notes put italics in a display context | concentrated in: longevityClinic 3, artEcommerce 1, ebikeUrban 1, modularSofa 1 — where it is a category convention it is not a tell. Italic *pull quotes* are excluded: they are a typographic norm, not this tell |
| mono type for "technical" flavour | **6/118** sites have a measured mono face | concentrated in: corporate-website 3, fintechApps 2, jobBoards 1 — do NOT ban it there |
| slop-flagged font | **39/118** sites use one (most common: Inter on 9) | measured does not mean safe, but "no real site uses these" is false — it needs a reason, not a ban |
| single type family | **54 two-family vs 64 single** | single-family wins in some categories; count yours |
